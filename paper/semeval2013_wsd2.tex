...
\title{Parameter optimisation for Memory-based Cross-Lingual Word-Sense Disambiguation}
...
The size of the local context, the inclusion of global context features, and the inclusion of syntactic features are all feature selection choices to the system. These allow for a variety of combinations to be tested. In addition, each word expert is a $k$-nearest neighbor classifier that can take on many hyperparameters beyond $k$. In the present study we performed both optimizations for all word experts, but the optimizations were not performed jointly: we first optimized classifier hyperparameters on the basis of the examples extracted from our parallel corpus. Subsequently we optimized feature selection on the basis of the trial data provided for the task. As has been argued before \cite{Host+02}, the joint search space of feature selection and hyperparameters is prohibitively large. Our current setup runs the risk of finding hyperparameters that are not optimal for the feature selection selected in the second optimization step. Our final results indeed show that only feature selection produced 

To optimize the choice of hyperparameters per word expert, a heuristic parameter search algorithm \cite{PARAMSEARCH} was used that implements wrapped progressive sampling: it performs a large number of experiments with many hyperparameter setting combinations on small samples of training data, and then progressively zooms in on combinations estimated to perform well with larger samples of the training data. As a control run we also trained word experts with default hyperparameters, i.e. with $k=1$ and with all other hyperparameters at their default values as specified in the TiMBL implementation.

Trial data for the twenty nouns was made available by the task organisers; we used this data for optimising the selection of features. Per word expert we select the feature selection with the highest score on this development set. This is done separately for both evaluation metrics in the task.

\section{Experiments \& Results}

To assess the accuracy of a certain configuration of our system as a whole, we take the average over all word experts. An initial experiment on the trial data explores the impact of different context sizes, with parameter optimisations on the classifiers. The results, shown in Figure~\ref{figcontext}, clearly indicate that on average the classifiers perform best with a local context of just one word to the left and one to the right of the word to be disambiguated. Larger context sizes have a negative impact on average accuracy. The same result holds if parameter optimisation is not performed.
...
We submitted three configurations of our system, the maximum number of runs, to the shared task. Adding lemma features to the local context window of three words proves beneficial in general, as shown in Table~\ref{tabtrialresults}. This is therefore the first configuration we submitted (\texttt{c1l}). As second configuration (\texttt{c1lN}) we submitted the same configuration without parameter optimisation on the classifiers.
...
The third configuration (\texttt{var}) we submitted selects \emph{per word expert} the configuration (always with parameter optimisation on the classifier) that has the highest score on the trial data, and thus mixes all kinds of configurations. This optimal choice always yields the highest scores on the trial data, but carries the risk of overfitting. Results on the trial data are shown in Table~\ref{tabpar}.
